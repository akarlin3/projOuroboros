...
\begin{itemize}
  ...
  \item Under oracle scoring the weak form rescues order recovery to a common 20 dB bracket; on a fair grid Tikhonov beats it only at high order.
  \item Oracle brackets are identifiability ceilings: deployment-realistic selection (noisy data only) degrades every method by 15--30 dB and breaks the monotone-in-$\alpha$ ordering.
...
\begin{table}[htbp]
\centering
\caption{Fair common-grid noise-mitigation comparison: recovery breakdown brackets $[$Fail, Succeed$]$ for all four methods on the \emph{identical} $5$ dB grid ($\{10,\dots,60\}$ dB), $500$ realizations per cell, identical seeds, and identical clean-derivative oracle scoring. A bracket $[a,b]$ means success rate $<95\%$ at $a$ dB and $\ge 95\%$ at $b$ dB. Tikhonov $\lambda$ is SNR-tuned ($\log_{10}\lambda = 3.2 - 0.05\,\mathrm{SNR}$); Ensemble uses $B=10$ bootstraps.}
...
\begin{itemize}
  ...
  \item \textbf{Ensemble-SINDy and Tikhonov Pre-smoothing:} On the fair common grid, bootstrapping (Ensemble-SINDy) provides essentially no improvement over naive pointwise GL ($[30,35]$/$[35,40]$/$[40,45]$ dB, identical to pointwise) because bagging pointwise estimates still inherits pointwise noise amplification. Tikhonov pre-smoothing is strongly order-dependent: it is worse than weak-form at low order ($[30,35]$ dB at $\alpha_t=0.5$, $[25,30]$ dB at $\alpha_t=0.7$) but \emph{beats} weak-form at high order, recovering $\alpha_t=0.9$ down to $[{<}10,10]$ dB ($99.6\%$ at $10$ dB) where heavy smoothing preserves the large-amplitude high-order signal. Its success curve is non-monotone near threshold (e.g.\ $\alpha_t=0.7$: $87.4\%$ at $15$ dB, dropping to $16.8\%$ at $20$ dB before recovering), making it less predictable than the weak form. Thus weak-form is the best \emph{uniform} remedy---the only method that recovers every order at $\le 20$ dB---but Tikhonov is competitive-to-better specifically at high order.
\end{itemize}

\subsection{Deployment-Realistic Order Selection}
\label{sec:realistic}
All brackets above are produced by an oracle criterion that scores candidates against the clean ground-truth derivative evaluated at the true order---side information unavailable in practice. To measure what is actually deployable, we re-run the entire pointwise and weak-form sweep (same $5$ dB grid, same $500$ realizations, same seeds, same candidate grid and exact-match criterion) under a single principled rule that uses only the noisy data: held-out $R^2$ on a noisy validation split, where each candidate's model is fit on the noisy training split and scored on the noisy validation split against \emph{that candidate's own} GL derivative (pointwise) or weak projection (weak-form) of the noisy validation data. This is the literal deployable analog of the oracle rule (clean trajectory $\to$ noisy data, true order $\to$ candidate order). The rule was fixed in advance; we report its output as-is.

The consequences are substantial (Table~\ref{tab:realistic}). First, both methods degrade by roughly $15$--$30$ dB: the pointwise succeed edges move from $35/40/45$ dB to $60/70/65$ dB, and the weak-form edges move from a uniform $20$ dB to $45/45/35$ dB. The oracle brackets are therefore genuine identifiability ceilings, not deployable thresholds. Second, the clean monotonic-in-$\alpha$ ordering does not survive: realistic pointwise selection is non-monotone in $\alpha$ ($\alpha_t=0.7$ becomes the hardest, not $\alpha_t=0.9$), i.e.\ roughly flat at $55$--$70$ dB, while realistic weak-form selection \emph{inverts}, with the high-order case $\alpha_t=0.9$ now the easiest ($[30,35]$ dB) and the low-order case the hardest. The low-$\alpha$-easiest ordering predicted by $A(\alpha)$ is thus specific to oracle scoring. Third, despite this degradation, the qualitative advantage of the weak form survives in magnitude: realistic weak-form ($35$--$45$ dB) still beats realistic pointwise ($60$--$70$ dB) by $15$--$30$ dB at every order, so the weak form remains the only realistically deployable selector. We also note that the realistic weak-form success curves are non-monotone (criterion-fragile) near their thresholds, consistent with the noise-floor analysis above.

\begin{table}[htbp]
\centering
\caption{Oracle (identifiability-ceiling) versus deployment-realistic recovery brackets $[$Fail, Succeed$]$ on the primary system, same $5$ dB grid / $500$ realizations / seeds. The realistic rule uses only noisy data (held-out noisy $R^2$ with candidate-consistent targets). A dagger ($\dagger$) marks a non-monotone success curve near threshold; the bracket uses the stable succeed edge.}
\label{tab:realistic}
\begin{tabular}{lcccc}
\toprule
& \multicolumn{2}{c}{Pointwise GL} & \multicolumn{2}{c}{Weak-Form GL} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
True $\alpha_t$ & Oracle & Realistic & Oracle & Realistic \\
\midrule
0.5 & [30, 35] & [55, 60] & [15, 20] & [40, 45]$^\dagger$ \\
0.7 & [35, 40] & [65, 70] & [15, 20] & [40, 45] \\
0.9 & [40, 45] & [60, 65] & [15, 20] & [30, 35] \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Conclusion}
This study evaluated the identifiability, stability, and noise fragility of data-driven modeling pipelines on a vascularized stroma reaction-diffusion model. While fractional SINDy displays clean sensitivity and specificity in the noise-free limit, it is practically fragile under realistic noise. Under oracle scoring (against the clean ground-truth derivative), the weak-form GL SINDy acts as a robust general-purpose remedy: it extends temporal order recovery down to a common bracket of $[15\text{ dB}, 20\text{ dB}]$ for all three orders $\alpha_t \in \{0.5, 0.7, 0.9\}$ (indistinguishable at the $5$ dB grid resolution), a $15$--$25$ dB improvement over naive pointwise GL, whose recovery threshold rises monotonically with the order---breaking down at brackets of $[30\text{ dB}, 35\text{ dB}]$, $[35\text{ dB}, 40\text{ dB}]$, and $[40\text{ dB}, 45\text{ dB}]$ for $\alpha_t = 0.5$, $0.7$, and $0.9$. We are careful to frame these as identifiability ceilings: under a deployment-realistic selection rule that uses only noisy data, every method degrades by $15$--$30$ dB and the monotone-in-$\alpha$ ordering does not survive, though the weak form remains the only deployable selector; and on a fair common grid, SNR-tuned Tikhonov pre-smoothing surpasses the weak form specifically at high order. These identifiability, amplification, and rescue findings reproduce on an independent fractional Van der Pol benchmark. Furthermore, we demonstrated that data-driven Lyapunov exponent estimators (e.g., Rosenstein) produce spurious positive exponents on stable transients and noise floors, making analytical tangent-space integration (Benettin) mandatory for reliable stability characterization---the same lesson, in a different guise, that data-driven diagnostics require model-based anchoring. These results suggest that regularized or weak-form fractional SINDy formulations should be paired with rigorous tangent-space diagnostics when modeling and diagnosing complex, non-equilibrium systems.

\section*{Data and Code Availability}
% TODO (pre-submission): create the Zenodo deposit (mint a DOI from the tagged GitHub
%   release) and replace the \zenododoi macro at the top of this file with the real DOI.
%   The macro is intentionally left as <PENDING>; do NOT fabricate a DOI.
The simulation codes, data generation scripts, and plotting routines developed for this study are available on GitHub at \url{https://github.com/akarlin3/projOuroboros} and will be archived on Zenodo under DOI: \href{https://doi.org/\zenododoi}{\texttt{\zenododoi}} (\textbf{DOI pending Zenodo deposit at submission}).
